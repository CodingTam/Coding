...
class UserForm(QWidget):
    ...
    def initUI(self):
        ...
        # Sixth Tab - DB2 Table Data
        self.tab6 = QWidget()
        self.tabs.addTab(self.tab6, "DB2 Data Table")
        self.init_tab6(self.tab6)
    ...
    def init_tab6(self, tab):
        ...
        # DB2 Table
        self.db2_table = QTableWidget()
        self.db2_table.setRowCount(10)
        self.db2_table.setColumnCount(5)  # Adjust based on your table structure
        self.db2_table.setHorizontalHeaderLabels(["Column1", "Column2", "Column3", "Column4", "Column5"])

        table_layout.addWidget(self.db2_table)
        table_widget.setLayout(table_layout)
        scroll_area.setWidget(table_widget)
        scroll_area.setWidgetResizable(True)
        ...
        # Load Button
        load_button = QPushButton("Load DB2 Data")
        load_button.clicked.connect(self.load_db2_data)
        layout.addWidget(load_button)

        tab.setLayout(layout)

    def load_db2_data(self):
        try:
            dsn = "DATABASE=db_name;HOSTNAME=hostname;PORT=50000;PROTOCOL=TCPIP;UID=username;PWD=password;"
            ...
            while True:
                ...
                if not row:
                    break
                self.db2_table.insertRow(row_number)
                for col_number, key in enumerate(row):
                    self.db2_table.setItem(row_number, col_number, QTableWidgetItem(str(row[key])))
                row_number += 1

            ibm_db.close(conn)
            print("Data loaded successfully from DB2.")
        except Exception as e:
            print(f"Error loading data from DB2: {e}")
