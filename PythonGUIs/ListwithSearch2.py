...
def submit_form(listboxes):
    ...
    for label, listbox in listboxes:
        ...
        if label == 'Right Listbox' and selected_items:
            selected_columns.extend(selected_items)  # Add these as column names
        elif selected_items:
            condition = f"{label} IN ({', '.join([f'\'{value}\'' for value in selected_items])})"
            query_conditions.append(condition)

    columns = ', '.join(selected_columns) if selected_columns else '*'
    if query_conditions:
        query = f"SELECT {columns} FROM mytable WHERE {' AND '.join(query_conditions)}"
    else:
        query = f"SELECT {columns} FROM mytable"
