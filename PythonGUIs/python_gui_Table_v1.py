...
def load_data_from_db(table):
    try:
        ...
        # Fetch column headers dynamically from the query result
        columns = [desc[0] for desc in ibm_db.fetch_assoc(stmt).keys()]
        table.setColumnCount(len(columns))
        table.setHorizontalHeaderLabels(columns)

        # Reset the table and load data
        table.setRowCount(0)
        row = 0
        ...
        while result:
            ...
            for col_index, col_name in enumerate(columns):
                table.setItem(row, col_index, QTableWidgetItem(str(result[col_name])))
            result = ibm_db.fetch_assoc(stmt)
            row += 1
        ibm_db.close(conn)
